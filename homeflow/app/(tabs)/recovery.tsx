/**
 * Recovery Tab Screen — Post-Surgery Recovery Plan
 *
 ...
 *
 * Tab: Recovery (between Health and Chat Helper)
 */
...
function RecoveryTimeline({ surgeryDateStr }: { surgeryDateStr: string }) {
  ...
  const statusLabel =
    daysPost < 0  ? 'Surgery upcoming' :
    daysPost === 0 ? 'Day of surgery' :
    daysPost === 1 ? '1 day post-op' :
    `Day ${daysPost} post-op`;

  return (
    <View style={[tlStyles.card, { backgroundColor: c.card }]}>
      <View style={tlStyles.statusRow}>
        <Text style={[tlStyles.statusLabel, { color: c.textPrimary }]}>{statusLabel}</Text>
        {daysPost >= 0 && daysPost <= 42 && (
          <Text style={[tlStyles.statusSub, { color: c.textSecondary }]}>
      ...
      {/* Progress bar */}
      <View style={[tlStyles.track, { backgroundColor: c.secondaryFill }]}>
        <View
          style={[
            tlStyles.fill,
      ...
      {/* Milestone labels */}
      <View style={tlStyles.milestonesRow}>
        {milestones.map(m => {
          ...
          return (
            <View
              key={m.day}
              style={[tlStyles.milestone, { left: `${pct}%` as `${number}%` }]}
            >
              <View
                style={[
                  tlStyles.dot,
...
function DietActivityContent() {
  ...
  return (
    <View style={{ gap: 4 }}>
      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Diet</Text>
      <BulletItem>Resume your regular diet.</BulletItem>
      <BulletItem>Encourage well-balanced, nutritious meals.</BulletItem>

      <View style={sc.divider} />

      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Activity</Text>
      <BulletItem>
...
function CatheterContent() {
  ...
  return (
    <View style={{ gap: 4 }}>
      <View style={[sc.infoBox, { backgroundColor: c.accent + '12', borderColor: c.accent + '30' }]}>
        <IconSymbol name="info.circle.fill" size={14} color={c.accent} />
        <Text style={[sc.infoText, { color: c.textSecondary }]}>
      ...
      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Daily Use</Text>
      ...
      <BulletItem>Secure the catheter with a strap or StatLock to prevent accidental pulling.</BulletItem>

      <View style={sc.divider} />

      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Balloon Removal</Text>
      <StepItem number={1}>Use a syringe to deflate the balloon fully before attempting removal.</StepItem>
      <StepItem number={2}>Up to 80 mL of fluid may need to be withdrawn from the balloon.</StepItem>
      <StepItem number={3}>Remove the catheter gently only after the balloon is completely deflated.</StepItem>
    </View>
  );
}
...
function MedicationsContent() {
  ...
  return (
    <View style={{ gap: 4 }}>
      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Pain Management</Text>
      <BulletItem>
      ...
      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>Stool Softeners</Text>
      <BulletItem>Options include: Colace, MiraLax, Senna, Dulcolax, or Milk of Magnesia.</BulletItem>
      <BulletItem><Text style={{ fontWeight: '600' }}>Stop if loose stools develop.</Text></BulletItem>
    </View>
  );
}
  ...
  return (
    <View style={{ gap: 4 }}>
      <View style={[sc.infoBox, { backgroundColor: c.accent + '12', borderColor: c.accent + '30' }]}>
        <IconSymbol name="info.circle.fill" size={14} color={c.accent} />
        <Text style={[sc.infoText, { color: c.textSecondary }]}>
          Temporary urinary incontinence is common after HoLEP. Kegel exercises strengthen
          the pelvic floor muscles and help restore control.
        </Text>
      </View>

      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>How to do them</Text>
      <BulletItem>Identify the muscles you use to stop the flow of urine or hold back gas.</BulletItem>
      <StepItem number={1}>Squeeze (contract) those pelvic floor muscles firmly.</StepItem>
      <StepItem number={2}>Hold for 3 seconds. Build up to 10 seconds over time.</StepItem>
      <StepItem number={3}>Release fully and rest for a moment.</StepItem>
      <StepItem number={4}>Repeat 10 times per session, 3–8 sessions daily.</StepItem>

      <View style={sc.divider} />

      <BulletItem><Text style={{ fontWeight: '600' }}>Stop immediately</Text> if exercises cause pain.</BulletItem>
      <BulletItem>Seek guidance from your care team if there is no improvement after several weeks.</BulletItem>
      ...
      >
        ...
        <Text style={[sc.checkLabel, { color: checked ? c.accent : c.textSecondary }]}>
...
function WarningSignsContent() {
  ...
  return (
    <View style={{ gap: 4 }}>
      <View style={[sc.infoBox, { backgroundColor: amber + '15', borderColor: amber + '35' }]}>
        <IconSymbol name="exclamationmark.triangle.fill" size={14} color={amber} />
        <Text style={[sc.infoText, { color: c.textPrimary }]}>
          <Text style={{ fontWeight: '600' }}>Call the clinic or go to the emergency room</Text>
      ...
      <Text style={[sc.groupLabel, { color: c.textSecondary }]}>What is normal</Text>
      <BulletItem>
        Some blood in the urine for{' '}
        <Text style={{ fontWeight: '600' }}>up to 2 weeks</Text> is expected and normal.
      </BulletItem>
      <BulletItem>
  ...
  return (
    <View style={{ gap: 8 }}>
      ...
      <Pressable
        ...
        style={[sc.callButton, { backgroundColor: c.accent }]}
        accessibilityRole="button"
        accessibilityLabel="Call Urology Clinic"
      >
        <IconSymbol name="phone.fill" size={15} color="#FFFFFF" />
        <Text style={sc.callButtonText}>Call Urology Clinic</Text>
      </Pressable>
    </View>
  );
}
...
function ReviewedToggle({
  ...
  if (reviewed) {
    return (
      <View style={[rvStyles.done, { backgroundColor: c.semanticSuccess + '15' }]}>
        <IconSymbol name="checkmark.circle.fill" size={13} color={c.semanticSuccess} />
        <Text style={[rvStyles.doneText, { color: c.semanticSuccess }]}>Section reviewed</Text>
      </View>
    );
  }
  return (
    <Pressable onPress={onMark} style={rvStyles.btn}>
      <Text style={[rvStyles.btnText, { color: c.textTertiary }]}>Mark section as reviewed</Text>
    </Pressable>
  );
}
...
export default function RecoveryScreen() {
  ...
  function markReviewed(id: string) {
    setReviewedSections(prev => new Set(prev).add(id));
  }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]} edges={['top']}>
      ...
      >
        {/* ── Large title header (matches other tab screens) ─────────── */}
        <Text style={[styles.screenTitle, { color: c.textPrimary }]}>Recovery</Text>
        ...
        {/* ── Progress Timeline (post-surgery only) ─────────────────── */}
        ...
        <View style={[styles.footerCard, { backgroundColor: c.card }]}>
          <Text style={[styles.footerTitle, { color: c.textPrimary }]}>Have a question?</Text>
...
const sc = StyleSheet.create({
  ...
  divider: {
    height: StyleSheet.hairlineWidth, backgroundColor: '#3C3C4333', marginVertical: 10,
  },
  infoBox: {
